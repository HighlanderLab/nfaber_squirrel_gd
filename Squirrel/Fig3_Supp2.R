...
resSum <- rename(resSum, gRNAs = nGuides.TA)
resSum$lethal <- ifelse(as.logical(levels(resSum$lethal)[resSum$lethal]), "Embryonic lethal", "Both-sex infertility")
resSum$overexpressionOK <- ifelse(as.logical(levels(resSum$overexpressionOK)[resSum$overexpressionOK]), "Allowed", "Not allowed")
resSum$lethal <- factor(resSum$lethal, levels=c("Embryonic lethal", "Both-sex infertility"))
resSum$overexpressionOK <- factor(resSum$overexpressionOK, levels=c("Not allowed", "Allowed"))

# View(resSum)

# save(res, resSum, file = 'Fig3_Supp2.RData')
# load(file = 'Fig3_Supp2.RData')
...
ggsave(plot = p + PaperTheme, filename = "Fig3_Supp2.png",    height = PaperSize * 1.5, width = PaperSize * 1.5, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig3_Supp2_PPT.png", height = PreseSize * 1.5, width = PreseSize * 1.5, unit = "cm")
