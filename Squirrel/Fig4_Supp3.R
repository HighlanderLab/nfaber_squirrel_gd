...
input$supplement <- seq(0,0.5,0.05) # percent of populationsize supplemented per supplementation
...
input$daisyfield <- seq(0,60,4) # number of daisies in daisy field, -1 if no daisy drive. 
...
# save(res, resSum, file = 'Fig4_Supp3.RData')
# load(file = 'Fig4_Supp3.RData')
...
PaperTheme <- theme_bw(base_size = 11, base_family = "sans") + theme(legend.position = "right", strip.background = element_blank(),panel.grid = element_blank(), legend.title.align = 0.5)
PreseTheme <- theme_bw(base_size = 18, base_family = "sans")  + theme(legend.position = "right", strip.background = element_blank(),panel.grid = element_blank(), legend.title.align = 0.5)
...
heatMapData50 <- resSum[resSum$year==50,] # complete suppression in 50 years
heatMapData50$extinctRate <- heatMapData50$extinctMean/100

# Suppression rate after 50 years
p = heatMapData50 %>%
  ggplot() +
  geom_tile(mapping = aes(x = supplement, y = daisyfield, fill = extinctRate)) +
  scale_fill_viridis(option="viridis", name = "Suppresion rate") +
  theme_bw() +
  theme(legend.position="right", strip.background = element_blank()) +
  xlab(label = "Supplementation amount (%)") +
  ylab(label = "Daisyfield size")
p

ggsave(plot = p + PaperTheme, filename = "Fig4_Supp3.png",    height = PaperSize, width = PaperSize * 1.5, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig4_Supp3_PPT.png", height = PreseSize, width = PreseSize * 1.5, unit = "cm")
