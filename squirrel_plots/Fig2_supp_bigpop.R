...
input$nGuides <- c(0,1,4) # number of guide RNA target sites
...
inputs <- inputs[which((inputs$strategy==4 & inputs$nGuides>0 & inputs$nGuides.TA==0 & inputs$nG==100) |
                         (inputs$strategy==7 & inputs$nGuides==0 & inputs$nGuides.TA>0 & inputs$nG==10000) |
                         (inputs$strategy==7 & inputs$nGuides>0 & inputs$nGuides.TA>0 & inputs$nG==100)),]
View(inputs)
...
# save(res, resSum, file = "Fig2_supp_bigpop.RData")
# load(file = "Fig2_supp_bigpop.RData")
...
prez.plot <- resSum[(as.integer(levels(resSum$nGuides)[resSum$nGuides])==1) | (as.integer(levels(resSum$strategy)[resSum$strategy])==4),]
copydf <- prez.plot
copydf <- resSum

copydf$gd.type <- NA
copydf[copydf$strategy==4,]$gd.type <- "Homing"
copydf[copydf$strategy==7,]$gd.type <- "Homing & Cleave-and-rescue"
copydf[copydf$strategy==7 & as.integer(levels(copydf$nGuides)[copydf$nGuides])==0,]$gd.type <- "Cleave-and-rescue"
copydf$gd.type <- factor(copydf$gd.type, levels=c("Homing","Homing & Cleave-and-rescue","Cleave-and-rescue"))

# nGuides Census
p = copydf %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = nIndMean, colour = nGuides:nGuides.TA, linetype = gd.type)) +
  geom_ribbon(mapping = aes(x = year, ymin = nIndQL, ymax = nIndQU, fill = nGuides:nGuides.TA), alpha = .1) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Homing : Cleave-and-rescue gRNAs",guide=guide_legend(ncol=2, title.position = "top")) +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Homing : Cleave-and-rescue gRNAs",guide=guide_legend(ncol=2, title.position = "top")) +
  scale_linetype_manual(values = c(1,2,3),name = "Gene drive strategy",guide=guide_legend(ncol=1,order=1, title.position = "top")) +
...
ggsave(plot = p + PaperTheme, filename = "Fig2_supp_bigpop.png",    height = PaperSize, width = PaperSize * 2, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig2_supp_bigpop_PPT.png", height = PreseSize, width = PreseSize * 2, unit = "cm")
