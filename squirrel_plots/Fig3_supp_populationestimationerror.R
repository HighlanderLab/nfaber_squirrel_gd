...
input$pop.est.acc <- c(0,0.1,0.5) # percent of populationsize standard deviation in population size estimation
...
res$pop.est.acc <- factor(res$pop.est.acc, labels = c("0%","10%","50%"))
...
resSum = res %>%
  group_by(strategy,nGuides,supplement,suppInterval,daisyfield,Pn,X.shr.eff,nGuides.TA,pop.est.acc,year) %>%
  summarise(extinctMean = mean(extinct, na.rm = TRUE),
            ...
            nSuppMean = mean(supplemented, na.rm = TRUE),
            nSuppQL   = quantile(supplemented, na.rm = TRUE, probs = 0.025),
            nSuppQU   = quantile(supplemented, na.rm = TRUE, probs = 0.975),
...
# save(res, resSum, file = 'Fig3_supp_populationestimationerror.RData')
# load(file = 'Fig3_supp_populationestimationerror.RData')
...
resSum <- rename(resSum, Error = pop.est.acc)

# nGuides Census
p <- resSum %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = nIndMean, colour = Supplement)) +
  geom_ribbon(mapping = aes(x = year, ymin = nIndQL, ymax = nIndQU, fill = Supplement), alpha = .3) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Supplementation",guide=guide_legend(title.position = "top")) +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Supplementation",guide=guide_legend(title.position = "top")) +
  facet_grid(. ~ Error, labeller = label_both) +
  theme_bw() +
...
ggsave(plot = p + PaperTheme, filename = "Fig3_supp_populationestimationerror.png",    height = PaperSize*0.85, width = PaperSize*2, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig3_supp_populationestimationerror_PPT.png", height = PreseSize*0.85, width = PreseSize*2, unit = "cm")

# Supplementation
p = resSum[resSum$nSuppMean>0,] %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = nSuppMean, colour = Supplement)) +
  geom_ribbon(mapping = aes(x = year, ymin = nSuppQL, ymax = nSuppQU, fill = Supplement), alpha = .3) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Supplement",guide=guide_legend(title.position = "top")) +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Supplement",guide=guide_legend(title.position = "top")) +
  facet_grid(. ~ Error, labeller = label_both) +
  theme_bw() +
  theme(legend.position="bottom", strip.background = element_blank()) +
  xlab(label = "Year") +
  ylab(label = "Amount supplemented")
p

ggsave(plot = p + PaperTheme, filename = "Fig3_supp_populationestimationerrorsupp.png",    height = PaperSize*0.85, width = PaperSize*2, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig3_supp_populationestimationerrorsupp_PPT.png", height = PreseSize*0.85, width = PreseSize*2, unit = "cm")
