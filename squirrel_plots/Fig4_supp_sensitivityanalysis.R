...
input$max.mating.range <- c(150,600) # the maximum observed distance a male approaches a female from
input$mating.range.obs <- 30 # the number of observations which led to the observed max.mating.range
input$max.movement <- c(500,10000) # maximum migration distance in meter
input$plot <- FALSE
...
res$max.mating.range <- factor(res$max.mating.range)
res$max.movement <- factor(res$max.movement)
...
resSum = res %>%
  group_by(strategy,nGuides,supplement,suppInterval,daisyfield,Pn,X.shr.eff,nGuides.TA,K,max.mating.range,max.movement,year) %>%
...
# save(res, resSum, file = 'Fig4_supp_sensitivityanalysis.RData')
# load(file = 'Fig4_supp_sensitivityanalysis.RData')
...
p = resSum %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = nIndMean, colour = max.movement, linetype = max.mating.range)) +
  geom_ribbon(mapping = aes(x = year, ymin = nIndQL, ymax = nIndQU, fill = max.movement, linetype = max.mating.range), alpha = .1) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Migration range (m)") +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Migration range (m)") +
  scale_linetype_manual(values = c(1,2),name = "Mating range (m)",guide=guide_legend(ncol=1,order=1, title.position = "top")) +
  theme_bw() +
...
ggsave(plot = p + PaperTheme, filename = "Fig4_supp_sensitivityanalysis_PPT.png",    height = PaperSize, width = PaperSize * 1.5, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig4_supp_sensitivityanalysis.png", height = PreseSize, width = PreseSize * 1.5, unit = "cm")
