...
input$nG <- c(100,1000) # initial number carrying the gene drive
...
inputs <- inputs[which((inputs$strategy==4 & inputs$nGuides>0 & inputs$nGuides.TA==0 & inputs$nG==100) |
                         (inputs$strategy==7 & inputs$nGuides==0 & inputs$nGuides.TA>0 & inputs$nG==1000) |
...
# save(res, resSum, file = "Fig2.RData")
...
copydf[copydf$strategy==7 & copydf$nGuides==0,]$gd.type <- "Cleave-and-rescue"
...
ggsave(plot = p + PaperTheme, path = "final", filename = "Fig2_prez.png",    height = PaperSize, width = PaperSize * 2, unit = "cm")
ggsave(plot = p + PreseTheme, path = "final", filename = "Fig2_PPT.png", height = PreseSize, width = PreseSize * 2, unit = "cm")
