...
input$K <- c(3000,30000) # carrying capacity of offspring
...
save(res, resSum, file = "Fig5_panmictic.RData")
# load(file = "Fig5_panmictic.RData")
...
## run one
# debug(gd.comp); res <- gd.comp(input = inputs[1, ])
...
# save(res, resSum, file = 'Fig5_spatial.RData')
# load(file = "Fig5_spatial.RData")
...
PreseTheme <- theme_bw(base_size = 18, base_family = "sans")  + theme(legend.position = "right", strip.background = element_blank(),panel.grid = element_blank())
...
res.spatial <- res
resSum.spatial <- resSum
resSum.spatial$type <- "Spatial"

load(file = "Fig5_panmictic.RData")
res.panmictic <- res
resSum.panmictic <- resSum
resSum.panmictic$type <- "Randomly mating"

res <- bind_rows(res.panmictic,res.spatial)
resSum <- bind_rows(resSum.panmictic,resSum.spatial)

# nGuides Census
p = resSum %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = pIndMean, colour = K, linetype = type)) +
  geom_ribbon(mapping = aes(x = year, ymin = pIndQL, ymax = pIndQU, fill = K, linetype = type), alpha = .1) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Population size",guide=guide_legend(ncol=1, title.position = "top")) +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Population size",guide=guide_legend(ncol=1, title.position = "top")) +
  scale_linetype_manual(values = c(1,2),name = "Model",guide=guide_legend(ncol=1, title.position = "top")) +
  theme_bw() +
  theme(legend.position="bottom", strip.background = element_blank()) +
  ylim(c(0, NA)) +
  xlab(label = "Year") +
  ylab(label = "Population size (% of carrying capacity)")
p

ggsave(plot = p + PaperTheme, filename = "Fig5.png",    height = PaperSize, width = PaperSize * 1.5, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig5_PPT.png", height = PreseSize, width = PreseSize * 1.5, unit = "cm")
