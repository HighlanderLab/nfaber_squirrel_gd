...
setwd(dir = 'YOUR_DIRECTORY')
source(file = '../gene_drive_spatial.R')
...
input$suppScheme <- c(1,2,3,4,5) ## 1 = Mean of population location
                      ## 2 = Modus population location
                      ## 3 = Placed randomly across area in 1 group
                      ## 4 = Placed randomly across area in 10 groups 
                      ## 5 = Moving front of 10 steps in 10 groups 
...
input$Pn <- 0.02 # c(0,0.02,0.1,0.5) # NHEJ probability
input$daisyfield <- 30 # c(4,10,30,60) # number of daisies in daisy field, -1 if no daisy drive. 
...
## run in parallel
res <- foreach(rowNum = 1:nrow(inputs), .packages = c('reshape2','tidyverse'), .verbose = TRUE) %dopar% { gd.comp(input = inputs[rowNum, ]) }
...
res$suppScheme <- factor(res$suppScheme, labels = c("Mean","Mode","Random","Random groups","Moving front"))
...
resSum = res %>%
  group_by(strategy,nGuides,supplement,suppInterval,daisyfield,Pn,X.shr.eff,nGuides.TA,suppScheme,year) %>%
...
# save(res, resSum, file = 'Fig6.RData')
# load(file = 'Fig6.RData')
...
PaperTheme <- theme_bw(base_size = 11, base_family = "sans") + theme(legend.position = "bottom", strip.background = element_blank(),panel.grid = element_blank())
PreseTheme <- theme_bw(base_size = 18, base_family = "sans")  + theme(legend.position = "bottom", strip.background = element_blank(),panel.grid = element_blank())
...
spatial.res <- res
spatial.resSum <- resSum
spatial.resSum$type <- "Spatial"

load(file = '../Squirrel/Fig4.RData')
panmictic.res <- res
panmictic.resSum <- resSum

baseline.resSum <- panmictic.resSum[resSum$supplement=="10%" & resSum$daisyfield==30,]
baseline.resSum <- do.call("rbind", replicate(5, baseline.resSum, simplify = FALSE))
baseline.resSum$suppScheme <- c(rep("Mean",101),rep("Mode",101),rep("Random",101),rep("Random groups",101),rep("Moving front",101))
baseline.resSum$type <- "Panmictic"

total.resSum <- rbind(spatial.resSum,baseline.resSum)
total.resSum$suppScheme <- factor(total.resSum$suppScheme, levels = c("Mean","Mode","Random","Random groups","Moving front"), labels = c("Mean","Mode","Random","Random groups","Moving front"))
total.resSum$type <- factor(total.resSum$type, levels = c("Spatial","Panmictic"))

# nGuides Census
p = total.resSum %>%
  ggplot() +
  geom_line(mapping = aes(x = year, y = nIndMean, colour = supplement, linetype = type)) +
  geom_ribbon(mapping = aes(x = year, ymin = nIndQL, ymax = nIndQU, fill = supplement, linetype = type), alpha = .1) +
  scale_colour_viridis(option="viridis", discrete=TRUE, name = "Supplement",guide=guide_legend(ncol=1,order=2, title.position = "top")) +
  scale_fill_viridis(option="viridis", discrete=TRUE, name = "Supplement",guide=guide_legend(ncol=1,order=2, title.position = "top")) +
  scale_linetype_manual(values = c(1,2),name = "Model",guide=guide_legend(ncol=1,order=1, title.position = "top")) +
  facet_grid(. ~ suppScheme, labeller = label_value) +
  theme_bw() +
...
ggsave(plot = p + PaperTheme, filename = "Fig6.png",    height = PaperSize, width = PaperSize*2.5, unit = "cm")
ggsave(plot = p + PreseTheme, filename = "Fig6_PPT.png", height = PreseSize, width = PreseSize*2.5, unit = "cm")
