...
library(viridis)
# install.packages(pkg = "Rccp")
# install.packages(pkg = "RccpArmadillo")
...
gd <- function(input) {
  ...
  reproductive <- function(pop, strategy) {
    ...
    if (nrow(pop)>0) {
      ...
      if (strategy==1) {
        ...
        ## no sex-reversal
        pop$p.sex <- pop$g.sex
        ## females homozygous for gene-drive allele are infertile and toxin but no antidote animals are sterile
        if (position=='intron') {
  ...
  ## Create population
  create <- function(nInit, nInit.vec, nG, Pr, area.side){

    pop <- data.frame(id=1:nInit, fid=0, mid=0)
    pop$age <- rep(rep(c(genTime,genTime*2),2),nInit.vec)
    
    ## location
    pop$x.loc <- runif(nInit, min = 0, max = area.side)
    pop$y.loc <- runif(nInit, min = 0, max = area.side)
  ...
  reproduce <- function(pop, m){
    ...
    if (length(rep.f.ind)>0 & length(rep.m.ind)>0) {
      ...
      A = cbind(pop$x.loc[rep.f.ind],pop$y.loc[rep.f.ind]) # x, y coordinates for replicating females
      B = cbind(pop$x.loc[rep.m.ind],pop$y.loc[rep.m.ind]) # x, y coordinates for replicating males
      paired.distances <- fastPairDist(A,B) # distance between females and males
      
      # probability of males approaching females, then random pick out of the ones that approach
      coming.over.males <- RBF(paired.distances,sqrt(-log(1/mating.range.obs))/max.mating.range) > matrix(runif(length(rep.f.ind)*length(rep.m.ind)), nrow = length(rep.f.ind))
      picked.males <- apply(coming.over.males,1,FUN=function(x) ifelse(length(which(x)>0),ifelse(length(which(x))==1,which(x),sample(which(x),1)),NA))
      rep.f.ind <- rep.f.ind[!is.na(picked.males)] # remove females who are not in range of fertile male
      rep.m.ind <- rep.m.ind[picked.males[!is.na(picked.males)]] # update males and remove NAs
    ...
    if (n.offspr > 0){
      ...
      offspring$x.loc <- rep.f.df$x.loc
      offspring$y.loc <- rep.f.df$y.loc
  ...
  supplementation <- function(pop, amount) {
    ...
    ## Location
    supp.pop <- placement(pop,supp.pop,suppScheme)
    ...
    if (plot){
      plot.locs(supp.pop,"supp_timepoint",t-1)
    }
  ...
  migrate <- function(pop,moving,average.distance,max.steps) {
    ## move individuals sequentially
    for (ind in moving){ 
      pop.locations <- cbind(pop$x.loc[-ind],pop$y.loc[-ind])
      ...
      while (!valid) {
        ## Move into a random direction and distance
        direction <- runif(1)*2*pi
        distance <- distance + rgamma(1,shape=5,scale=average.distance/5)
        
        new.x <- pop$x.loc[ind] + distance*cos(direction)
        new.y <- pop$y.loc[ind] + distance*sin(direction)
        
        ## check the movements are wihtin the area.side
        if (new.x > 0 & new.x < area.side & new.y > 0 & new.y < area.side & distance <= max.movement) { 
          new.loc <- array(c(new.x,new.y),dim=c(1,2))
          distances <- fastPairDist(new.loc,pop.locations)
          local.density <- length(which(distances < home.range)) # calculate the local density
          
          ## if there is habitat available or if this is their last attempt
          if (local.density < max.density | attempt >= max.steps) { 
            pop[ind,c("x.loc","y.loc")] <- new.loc
            valid <- TRUE
          }
        } else { ## set distance to zero and try again
          distance <- 0
        }
        attempt <- attempt + 1
      }
  ...
  movement <- function(pop) {
    ## assign migration
    locations <- cbind(pop$x.loc,pop$y.loc)
    distances <- fastDist(locations) 
    local.density <- apply(distances,1,function(x) {length(which(x < home.range))-1})
    chance.migrate <- sapply(local.density,p.migrate,simplify = TRUE)
    migrators <- runif(length(chance.migrate)) < chance.migrate
    
    emmigrants <- which(migrators)
    residents <- which(!migrators)

    ## emmigrant movement
    if (length(emmigrants>0)){
      if(length(emmigrants)>1) {
        emmigrants <- sample(emmigrants,size=length(emmigrants),replace=FALSE) # randomize order
      }
      pop <- migrate(pop,emmigrants,average.distance=3*home.range,50)
    }
    ## resident movement
    if (length(residents>0)){
      if(length(residents)>1) {
        residents <- sample(residents,size=length(residents),replace=FALSE) # randomize order
      }
      pop <- migrate(pop,residents,average.distance=home.range,1)
    }
    
    pop
  }
  
  placement <- function(pop,supp.pop,suppScheme) {
    ## Mean of population location
    if (suppScheme == 0){
      supp.pop$x.loc <- 0.5*area.side
      supp.pop$y.loc <- 0.5*area.side
    }
    ## Mean of population location
    else if (suppScheme == 1){
      supp.pop$x.loc <- mean(pop$x.loc)
      supp.pop$y.loc <- mean(pop$y.loc)
    }
    ## Modus population location
    else if (suppScheme == 2){
      supp.pop$x.loc <- getmode(round(pop$x.loc/100, digits = 0)*100)
      supp.pop$y.loc <- getmode(round(pop$y.loc/100, digits = 0)*100)
    }
    ## Placed randomly across area in 1 group
    else if (suppScheme == 3){
      supp.pop$x.loc <- runif(1,0,area.side)
      supp.pop$y.loc <- runif(1,0,area.side)
    }
    ## Placed randomly across area in 10 groups 
    else if (suppScheme == 4){
      supp.pop <- mutate(supp.pop, group=rep_along(1:nrow(supp.pop),1:10)) %>%
        group_by(group) %>%
        mutate(x.loc = runif(1,0,area.side)) %>%
        mutate(y.loc = runif(1,0,area.side))
      supp.pop <- as.data.frame(ungroup(supp.pop))
      supp.pop$group <- NULL
    }
    ## Moving front of 10 steps in 10 groups 
    else if (suppScheme == 5){
      supp.pop <- mutate(supp.pop, group=rep_along(1:nrow(supp.pop),1:10)) %>%
        group_by(group) %>%
        mutate(x.loc = runif(1,0,area.side))
      supp.pop <- as.data.frame(ungroup(supp.pop))
      supp.pop$group <- NULL
      supp.pop$y.loc <- ifelse(year[t]/suppInterval<10,
                               (year[t]+1)/suppInterval*(area.side/(10+1)),
                               area.side-area.side/(10+1))
    }
  ...
  p.migrate = function(local.density){
    ifelse(local.density <= max.density,0,1-max.density/local.density)
  }
  ...
  plot.locs <- function(pop,filename,t) {
    p = pop %>%
      ggplot() + 
      geom_point(mapping = aes(x = x.loc, y = y.loc), col = ifelse((pop$aut.m==-999|pop$aut.p==-999),'#fec004','#000000'), size = 1) +
      xlim(c(0, area.side)) +
      ylim(c(0, area.side)) +
      xlab(label = "Location (m)") +
      ylab(label = "Location (m)") +
      ggtitle(sprintf("Generation %i",t))
    p
    PreseTheme <- theme_bw(base_size = 18, base_family = "sans")  + theme(plot.title = element_text(hjust = 0.5),
                                                                          legend.position = "right", 
                                                                          panel.grid = element_blank(),
                                                                          panel.background = element_rect(fill="#808080"),
                                                                          panel.border = element_blank())
    ggsave(plot = p + PreseTheme, path = "plots", filename = paste(filename,sprintf("%i.png",t),sep=""), height = 15, width = 15, unit = "cm")
  }
  ...
  nT <- length(gen)-1
  area.side <- sqrt(K*pi*home.range^2)/sqrt(max.density)
  ...
  for (iter in 1:nIter) {
    ...
    t=1
    nInd <- nInit+nG
    pop <- create(nInit=nInit, nInit.vec=nInit.vec, nG=nG, Pr=Pr, area.side=area.side)
    if (plot){
      plot.locs(pop,"timepoint",0)
    }
